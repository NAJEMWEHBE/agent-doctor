...
### Security
- **A trusted `checks.json` is now read once and both hash-verified and executed from the same in-memory buffer.** Previously the file was read twice — once to compare its SHA-256 against the pinned hash, once to load the checks that run — leaving a small window in which a concurrent writer could swap the file between the two reads (verify the trusted bytes, execute different ones). `trustedChecks` now reads the file a single time and derives both the hash check and the parsed checks from that one buffer, so what is verified is exactly what runs. Defense-in-depth: applies only to a `checks.json` you have already trusted (gated since 0.6.0).
