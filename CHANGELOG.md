...
### Security
- **`agent-doctor trust` now discloses a `checks.json`'s full capability before pinning it, and the `sqlite3` fallback is read-only.** The consent line previously counted only `exec`/`mcp` probes as "run a command", so a `memwrite` probe (which can reach the `sqlite3` CLI's `.shell`/`.system`) and an `http` probe carrying `${ENV:VAR}` in its URL/headers (which exfiltrates that environment secret to a check-chosen URL) were both invisible in the summary. `trust` now counts `exec`/`mcp`/`memwrite` as command-capable and separately reports how many checks can send a secret. Independently, the `memwrite` probe's `sqlite3` CLI fallback now opens the database with `-readonly` (the `node:sqlite` and `python` backends were already read-only), denying that fallback any write/`ATTACH` side-effect. Both are defense-in-depth — they apply only to a `checks.json` you have already trusted (gated since 0.6.0), and change no default behavior.
